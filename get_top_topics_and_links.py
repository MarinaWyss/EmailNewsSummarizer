...
def summarize_email(email_text: str) -> str:
    ...
    prompt = generate_prompt(
        email_text,
        """
        Identify the key topics discussed in this email.
        Return the topics in a bulleted list along with links.
        """
    )
...
def batch_summarize_and_identify_topics(summaries: List[str], batch_size: int = 5) -> List[str]:
    ...
    for i in range(0, len(summaries), batch_size):
        ...
        prompt = generate_prompt(
            combined_batch,
            """
            Identify the most important and frequent topics discussed in these emails.
            Return the topics in a bulleted list along with links."""
        )
...
def final_summary_of_top_topics(batch_summaries: List[str]) -> str:
    ...
    prompt = generate_prompt(
        combined_summaries, 
        """Provide a final bulleted list of the overall 
        most important topics from these emails along with links."""
    )
    return openai_request(prompt)
...
def main() -> None:
    ...
    logger.info("Final Summary of Important Topics from the Week:")
